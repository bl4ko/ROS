...
void cloud_cb1(const pcl::PCLPointCloud2ConstPtr & cloud_in) {
    ...
    downsample_pointcloud(cloud_in, cloud);
    //pcl::fromPCLPointCloud2( * cloud_in, * cloud);
    

    do_pass_through_filter(cloud);

    pcl::PointCloud < pcl::Normal > ::Ptr cloud_normals(new pcl::PointCloud < pcl::Normal > );
    get_normals_for_cloud(cloud, cloud_normals);
    ...
    remove_plane(cloud, inliers_plane);

    extract_normals(cloud_normals, inliers_plane);


    // detect cylinder

    // deal with cylindrical component
    */
    //############################################
    ...
    *cloud_filtered2 = *cloud;


    ////////////// remove all major planes start
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_plane_tmp (new pcl::PointCloud<pcl::PointXYZRGB> ());
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_f (new pcl::PointCloud<pcl::PointXYZRGB>);
    pcl::PointCloud < pcl::Normal > ::Ptr cloud_normals_f(new pcl::PointCloud < pcl::Normal > );
    pcl::ExtractIndices < pcl::Normal > extract_normals_tmp;
    

    int i=0, nr_points = (int) cloud_filtered2->points.size ();
    //int max_iterations_planes = 1000;
    //int max_iterations_planes = 5;
    int max_iterations_planes = 10;
    //int max_iterations_planes = 100;
    int number_of_removed_planes = 0;
    // While 30% of the original cloud is still there
    while ((cloud_filtered2->points.size () > 0.3 * nr_points) && (number_of_removed_planes < max_iterations_planes)) {
    //while ((cloud_filtered2->points.size () > 0.1 * nr_points) && (number_of_removed_planes < max_iterations_planes)) {
        number_of_removed_planes++;

        // segment largest plane from remaining cloud
        ...
        */
        ...
        // Extract the planar inliers from the input cloud
        pcl::ExtractIndices<pcl::PointXYZRGB> extract_tmp;
        extract_tmp.setInputCloud (cloud_filtered2);
        extract_tmp.setIndices (inliers);
        extract_tmp.setNegative (false);

        // Get the points associated with the planar surface
        extract_tmp.filter (*cloud_plane_tmp);
        std::cout << "PointCloud representing the planar component: " << cloud_plane_tmp->points.size () << " data points." << std::endl;

        // Remove the planar inliers, extract the rest
        extract_tmp.setNegative (true);
        extract_tmp.filter (*cloud_f);
        *cloud_filtered2 = *cloud_f;

        /*
        // for normals
        extract_normals_tmp.setNegative(true);
        extract_normals_tmp.setInputCloud(cloud_normals2);
        extract_normals_tmp.setIndices(inliers);
        extract_normals_tmp.filter( * cloud_normals_f);
        *cloud_normals2 = *cloud_normals_f;
        */
    }

    ///////////// remove all major planes end

    // recompute cloud normals
    //get_normals_for_cloud(cloud_filtered2, cloud_normals2);
    ...
    */
    ...
    std::cout << "Cylinder radius: " << coefficients_cylinder->values[6] << std::endl;
    ...
    //if (inliers_cylinder->indices.size() < 2000) {
    if (inliers_cylinder->indices.size() < 700) {
        return;
    }
    ...
    /*
    pcl::PCLPointCloud2 outcloud_cylinder;
    pcl::toPCLPointCloud2 (*cloud, outcloud_cylinder);
    puby.publish (outcloud_cylinder);
    */
    // test
...
void 
cloud_cb_basic (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
    // memory test
    //printMemoryUsage();
...
cloud_cb(const pcl::PCLPointCloud2ConstPtr & cloud_blob) {
    ...
    */
    ...
    // TODO: no need to publish plane - only slows down the simulation
    /*
