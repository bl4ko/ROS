...
class Brain:
    ...
    def __init__(self):
        ...
        self.map_markers_timer = rospy.Timer(rospy.Duration(1), lambda event: brain.map_show_markers())
        self.faces_subscriber = rospy.Subscriber("/detected_faces", DetectedFaces, self.faces_callback)
    ...
    def nearest_neighbor_path(self,vertices, start_vertex):
        ...
        while unvisited_vertices:
            ...
            for vertex in unvisited_vertices:
                distance = math.sqrt((current_vertex[0] - vertex[0]) ** 2 + (current_vertex[1] - vertex[1]) ** 2)
            ...
            with self.detected_faces_lock:
                if len(self.detected_faces) > df:
                    ...
                    for new_face in new_faces:
                        ...
                        self.move_to_goal(new_face.x_coord, new_face.y_coord, new_face.rr_x, new_face.rr_y, new_face.rr_z, new_face.rr_w)
                        #stop for 5 seconds
                        rospy.sleep(2)
